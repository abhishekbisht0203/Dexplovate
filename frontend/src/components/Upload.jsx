        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm sticky top-6">
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-xl text-blue-600">
                  <span className="flex items-center gap-2">
                    <Eye className="h-5 w-5" />
                    PDF Preview
                  </span>
                  {previewPdf && (
                    <Button variant="ghost" size="sm" onClick={() => setPreviewPdf(null)} className="h-8 w-8 p-0">
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </CardTitle>
                ...
                {previewPdf ? (
                  <div className="space-y-4">
                    <div className="text-sm text-gray-600">
                      <p className="font-semibold truncate" title={previewPdf.file.split("/").pop()}>
                        {previewPdf.file.split("/").pop()}
                      </p>
                      <p>Uploaded: {new Date(previewPdf.uploaded_at).toLocaleDateString()}</p>
                    </div>
                    <div className="border rounded-lg overflow-hidden bg-gray-100">
                      <iframe src={previewPdf.url} className="w-full h-96" title="PDF Preview" />
                    </div>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleDownload(previewPdf)}>
                        <Download className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
